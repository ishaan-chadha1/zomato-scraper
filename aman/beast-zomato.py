...
def run_pipeline():
    CARD_SELECTOR = "div.jumbo-tracker"
    BASE_URL = 'https://www.zomato.com/bangalore/restaurants'
    ...
    for i in range(0, len(pending_areas), BATCH_SIZE):
        ...
        with sync_playwright() as p:
            ...
            for area in chunk:
                ...
                # === IRONCLAD SEARCH VALIDATION LOOP ===
                for attempt in range(3):
                    try:
                        print(f"Loading Zomato homepage (Attempt {attempt+1}/3)...")
                        page.goto(BASE_URL, wait_until='domcontentloaded', timeout=45000)
                        page.wait_for_timeout(2000)

                        # Try multiple ways to find the location box
                        try:
                            location_input = page.get_by_placeholder("Bengaluru")
                            location_input.wait_for(state='visible', timeout=3000)
                        except:
                        ...
                        # HUMAN TYPING: Type slowly to force the dropdown API to load
                        print(f"Typing '{area}'...")
                        location_input.press_sequentially(area, delay=150)

                        print("Waiting for dropdown suggestions...")
                        wait_time = 3500 + (attempt * 1500)
                        ...
                        # VALIDATION: Does the Filters button exist? 
                        filters_button = page.locator('text=Filters').first
                        if filters_button.is_visible():
                            print("✅ Search successful! Found neighborhood grid.")
                            search_success = True
                            break
                        else:
                            print("⚠️ Filters not found. We landed on a bad page. Retrying search...")
                            
                    except Exception as e:
                        print(f"⚠️ Error during search input: {e}")
                
                if not search_success:
                    print(f"❌ ERROR: Could not properly load neighborhood grid for {area} after 3 attempts. Skipping.")
                    continue 
                # ========================================
