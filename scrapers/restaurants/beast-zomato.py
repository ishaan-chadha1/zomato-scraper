# NOTE: Currently a copy of beast-scraper.py paths/layout; fork here if you need a variant pipeline.
import csv
import os
import random
import time
...
# Repo root = …/Zomato-Scraper (this file lives in scrapers/restaurants/)
REPO_ROOT = Path(__file__).resolve().parents[2]
DATA_REST = REPO_ROOT / "data" / "restaurants"
CONFIG_DIR = DATA_REST / "config"
CSV_DIR = DATA_REST / "csv"
AREAS_FILE = CONFIG_DIR / "areas.txt"
COMPLETED_LOG = CONFIG_DIR / "completed_areas.log"
AREA_PASS_CSV = CSV_DIR / "zomato_restaurants_3pass.csv"
MASTER_CSV = CSV_DIR / "zomato_master_unique.csv"
...
def run_pipeline():
    ...
    if not AREAS_FILE.exists():
        print(f"Error: {AREAS_FILE} not found! Create it and add your locations.")
        return

    if not COMPLETED_LOG.exists():
        COMPLETED_LOG.write_text("", encoding="utf-8")
